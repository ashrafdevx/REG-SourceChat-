...
async function initQdrantClient() {
  ...
  try {
    ...
    console.warn("No Qdrant client library is installed. Install @qdrant/js-client-rest for full Qdrant support.");
    throw err;
  }
}
...
export async function ensureCollection(collectionName, vectorSize = 1536) {
  ...
  try {
    ...
    if (c.getCollections) {
      ...
      const exists = (res.collections || []).some((x) => x.name === collectionName);
      ...
      console.warn("Qdrant client present but collection API shape not recognized.");
    }
